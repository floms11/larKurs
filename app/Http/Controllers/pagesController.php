...
{
    const maxResult = 5; // Максимальное количество полученых балов
	const minVideoLenght = 55; // Длина видео (примерно 95% от длины, что бы засчитать просмотр)
	...
	private function getRandomPage($userID, Request $request) {
		...
		if ($count > 0) {
			...
			DB::table('kurs')->where('id', $userID)->update(['endTime' => time()]);
		}
	...
	private function plusResult($userID) {
		$res = DB::table('kurs')->where('id', $userID)->get()[0]->result;
		$res += 1;
		if ($res < 0)
			$res = 0;
		if ($res > self::maxResult)
			$res = self::maxResult;
		
		DB::table('kurs')->where('id', $userID)->update(['result' => $res]);
	}
	...
	public function triger(Request $request, $page) {
		...
		switch ($page) {
			...
			case 'video': // Проверяем просмотр видео до конца
				...
				if (time() - $startTime >= $resInput && $resInput > self::minVideoLenght)
					self::plusResult($userID);
				break;
		}
	...
	public function trigerReg(Request $request) {
		...
		if ($userID < 1) {
			...
			$id = DB::table('kurs')->insertGetId(['name' => $name, 'startTime' => time()]);
	...
	public function start(Request $request) {
		$list = DB::select('select * from kurs where endTime > 0 order by endTime desc limit 10');
	...
	public function days(Request $request) {
		...
		if (!is_array($days)) {
			...
			$request->session()->put('daysTrue', $days[0]);
	...
	public function finish(Request $request) {
		$userID = $request->session()->get('id', 0);
		if ($userID > 0) {
			$usersData = DB::table('kurs')->where('id', $userID)->get();
